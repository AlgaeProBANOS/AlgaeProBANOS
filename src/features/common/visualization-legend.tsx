...
export function VisualizationLegend(props: VisualizationLegendProps): JSX.Element {
    ...
    ...new Set(
      Object.values(events).map((event: Event) => {
        const translatedEventKind =
          event.kind in vocabularies ? vocabularies[event.kind].label.default : event.kind;
  ...
  if (eventKindProperties.length > 0 || Object.keys(groupedEntities).length > 0) {
    return (
      <div className="border-intavia-neutral-400 grid max-w-[250px] cursor-default grid-cols-[min-content_auto] truncate bg-white dark:bg-memorise-gold-100 dark:text-memorise-dark p-1 text-sm">
        {content}
      </div>
    );
  } else {
