...
export default function DocumentElement(props: DocumentProps): JSX.Element {
  ...
  const [visibleFragment, setVisibleFragment] = useState(Object.values(sortedKeysByDates)[0]);
  ...
  const setInView = (inView: boolean, entry: IntersectionObserverEntry) => {
    if (inView) {
      ...
      if (selID != null) {
        ...
        if (selFrag && id === dateContext.director && dateContext.alignScrolling === true) {
          if (containerRef.current != null) {
            const currentScrollPosition = containerRef.current.scrollTop;

            const lastScrollY = parseInt(containerRef.current.getAttribute('lastScrollY') ?? '0');
            if (lastScrollY > currentScrollPosition) {
              setScrollingDown(false);
            } else {
              setScrollingDown(true);
            }
            containerRef.current.setAttribute('lastScrollY', currentScrollPosition.toString());
          }
    ...
    <div ref={containerRef} className="overflow-hidden overflow-y-scroll">
      {isLoading ? (
        ...
        sortedKeysByDates.map((e) => {
          return (
            <InView root={containerRef.current} onChange={setInView} threshold={0.2} key={e}>
              {({ ref }) => {
                return (
