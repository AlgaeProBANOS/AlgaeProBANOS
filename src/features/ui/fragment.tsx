...
export const FragmentElement = forwardRef<HTMLDivElement, FragementElementProps>(
    ...
    const fragementTextWithAnnotation = useMemo(() => {
      ...
      if (fragment != null && fragment.text != null) {
        ...
        if (mergedAnnotations.length > 0) {
          ...
          contentArray.push(fragment.text.substring(mergedAnnotations.at(-1)!.end + 1));
        } else {
      ...
      return contentArray;
    }, [fragment, id, searchContext.searchTerm, searchContext.isCasing, showAnnotation]);

    const content = useMemo(() => {
      return (
        <>
          <div className={`mb-1 font-bold ${selected === true ? 'bg-memorise-blue-100' : ''}`}>
            {fragment ? fragment.date : 'No Date'}
          </div>
          <div>{fragment?.text != null ? fragementTextWithAnnotation : 'No Text'}</div>
        </>
      );
